...
const CheckoutPage: React.FC = () => {
  ...
  const handleTipoEnvioChange = (nuevoTipoEnvio: TipoEnvio) => {
    ...
    // Ya no forzamos la forma de pago, el usuario puede elegirla.
  };
  ...
  const handlePlaceOrder = async () => {
    ...
    const domicilioSeleccionado = cliente?.domicilios.find(
      (d) => d.id === selectedDomicilioId
    );
    ...
    const maxTiempoEstimado = Math.max(
      ...
      ...cart.map(
        (item) =>
          (item.articulo as ArticuloManufacturadoResponse)
            .tiempoEstimadoMinutos || 0
      )
    );
    const tiempoAdicional = tipoEnvio === "DELIVERY" ? 10 : 0;
    const estimatedTime = new Date(
      new Date().getTime() + (maxTiempoEstimado + tiempoAdicional) * 60000
    );
    ...
    try {
      ...
      const response: PedidoResponse = await PedidoService.create(
        cliente.id,
        pedidoData
      );
      ...
      if (formaPago === "MERCADO_PAGO" && response.mpPreferenceId) {
        setSuccessMessage(
          `Pedido #${response.id} generado. Por favor, completa el pago.`
        );
        setPreferenceId(response.mpPreferenceId);
      } else {
        setSuccessMessage(
          `¡Tu pedido #${response.id} ha sido realizado con éxito!`
        );
        clearCart();
              ...
              {cart.length === 0 ? (
                ...
                cart.map((item) => (
                  ...
                  >
                    ...
                    <strong>
                      ${(item.articulo.precioVenta * item.quantity).toFixed(2)}
                    </strong>
                  </ListGroup.Item>
                ))
              )}
            ...
            <Card.Body>
              ...
              <Form.Group className="mb-3">
                ...
                <Form.Control
                  ...
                  value={
                    selectedSucursal
                      ? `${selectedSucursal.nombre} (${selectedSucursal.domicilio.calle} ${selectedSucursal.domicilio.numero})`
                      : "No seleccionada"
                  }
                />
              ...
              <Form.Group className="mb-3">
                ...
                <div>
                  ...
                  <Form.Check
                    ...
                    label={
                      <>
                        <FontAwesomeIcon icon={faStore} className="me-1" /> Take
                        Away
                      </>
                    }
                    name="tipoEnvio"
              ...
              {tipoEnvio === "DELIVERY" && (
                <Form.Group className="mb-3">
                  ...
                  <Form.Select
                    ...
                    onChange={(e) =>
                      setSelectedDomicilioId(Number(e.target.value))
                    }
              ...
              {!preferenceId ? (
                <Button
                  ...
                  disabled={
                    ...
                    !cliente ||
                    !selectedSucursal ||
                    (tipoEnvio === "DELIVERY" && !selectedDomicilioId)
                  }
                  className="w-100 mt-3"
                >
                  {submittingOrder ? (
                    <>
                      <Spinner as="span" size="sm" /> Procesando...
                    </>
                  ) : (
                    "Realizar Pedido"
                  )}
