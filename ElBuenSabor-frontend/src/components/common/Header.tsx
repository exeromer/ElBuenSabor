...
import { Navbar, Nav, Container, Button, Badge } from 'react-bootstrap'; // Añadido Badge para el conteo del carrito
...
import {
  ...
  faSignInAlt,
  faSignOutAlt,
  faClipboardList, // Icono para "Mis Pedidos"
} from '@fortawesome/free-solid-svg-icons'; // Eliminado faCog (no usado)
import { useCart } from '../../context/CartContext'; // Importa el hook para el carrito
import CartModal from '../cart/CartModal'; // Importa el componente del modal del carrito
...
const Header: React.FC<HeaderProps> = () => {
  ...
  return (
    // Barra de navegación principal de Bootstrap
    <Navbar bg="light" expand="lg" className="shadow-sm">
      <Container>
        {/* Marca de la Navbar, enlazada a la página de inicio */}
        <Navbar.Brand as={Link} to="/">El Buen Sabor</Navbar.Brand>
        {/* Botón de toggle para menús responsivos en dispositivos pequeños */}
        ...
        <Navbar.Collapse id="basic-navbar-nav">
          ...
          {/* Enlaces y botones de acción (alineados a la derecha) */}
          <Nav>
            {/* Botón/enlace del carrito */}
            <Nav.Link onClick={() => setShowCartModal(true)} style={{ cursor: 'pointer' }}>
              <FontAwesomeIcon icon={faShoppingCart} /> Carrito
              {/* Muestra el conteo de ítems del carrito como un badge */}
              {getCartItemCount() > 0 && (
            ...
            {!isAuthenticated ? (
              ...
              // Enlaces y botón para usuario autenticado (Perfil y Cerrar Sesión)
              <>
      ...
      {/* Modal del carrito, su visibilidad controlada por `showCartModal` */}
