 ...
 */
export const getArticulosInsumo = async (): Promise<ArticuloInsumo[]> => {
  try {
    const response = await apiClient.get<ArticuloInsumo[]>('/articulosinsumo');
    return response.data.filter(ai => ai.estadoActivo);
  } catch (error) {
